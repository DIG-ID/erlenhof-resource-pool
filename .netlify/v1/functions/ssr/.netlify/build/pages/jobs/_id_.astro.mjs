...
</p> <p class="mt-1">${job.createdBy.name} ${job.createdBy.surname} (${job.createdBy.email})</p> </div>`} <div class="flex justify-end gap-x-4 mt-6"> <!-- Se o job não estiver atribuído, mostra botão "Assign" --> ${!job.assigned ? renderTemplate`<form method="post" action="/api/jobs/assign"> <input type="hidden" name="jobId"${addAttribute(id, "value")}> ${renderComponent($$result3, "Button", Button, { "type": "submit", "className": "bg-green-600 text-white cursor-pointer", "size": "sm" }, { "default": async ($$result4) => renderTemplate` ${renderComponent($$result4, "CheckCircle", CheckCircle, { "size": 16 })} Apply to Job
` })} </form>` : renderTemplate`<!-- Se o job já estiver atribuído, mostra botão desativado -->
            ${renderComponent($$result3, "Button", Button, { "disabled": true, "class": "bg-gray-400 text-white cursor-not-allowed" }, { "default": async ($$result4) => renderTemplate` ${renderComponent($$result4, "CheckCircle", CheckCircle, { "size": 16 })} Taken
