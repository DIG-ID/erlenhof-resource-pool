...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": job.title }, { "default": async ($$result2) => renderTemplate` ${renderComponent($$result2, "DashboardWrapper", $$DashboardWrapper, {}, { "default": async ($$result3) => renderTemplate` ${maybeRenderHead()}<div class="flex w-full scroll-mt-16 flex-col rounded-lg border"> <div class="border-b px-4 py-3 flex flex-col lg:flex-row lg:justify-between lg:items-center gap-y-6 lg:gap-y-0"> <div class="flex flex-col lg:flex-row items-start lg:items-center gap-x-4 gap-y-2 lg:gap-y-0"> <h1 class="text-lg font-semibold break-all">${job.title}</h1> <span class="relative rounded bg-muted px-2 py-1 font-mono text-xs font-medium flex items-center gap-x-2"> ${renderComponent($$result3, "Fingerprint", Fingerprint, { "size": 16 })} ${id} </span> </div> <div class="flex flex-col lg:flex-row lg:items-center gap-y-2 lg:gap-y-0 gap-x-3 text-sm"> <span class="flex items-center gap-x-1"> ${renderComponent($$result3, "GalleryVerticalEnd", GalleryVerticalEnd, { "size": 16 })} <span class="font-medium">Pool:</span> ${job.pool || "Not provided"} </span> <span class="flex items-center gap-x-1"> ${renderComponent($$result3, "Award", Award, { "size": 16 })} <span class="font-medium">Status:</span> ${job.statuses || "Not provided"} </span> <span class="flex items-center gap-x-1"> ${renderComponent($$result3, "CalendarDays", CalendarDays, { "size": 16 })} <span class="font-medium">Job Date:</span> ${job.date ? formatDate(job.date) : "Not provided"} </span> <span class="flex items-center gap-x-1"> ${renderComponent($$result3, "CheckCircle", CheckCircle, { "size": 16 })} <span class="font-medium">Assigned:</span> ${job.assigned ? "Yes" : "No"} </span> </div> </div> <div class="p-6"> <div class="mb-6"> <p class="font-medium flex items-center gap-x-2">${renderComponent($$result3, "FileText", FileText, { "size": 16 })} Description</p> <p class="leading-7 mt-2">${job.description || "Not provided"}</p> </div> <div class="mb-6"> <p class="font-medium flex items-center gap-x-2">${renderComponent($$result3, "FileText", FileText, { "size": 16 })} Notes</p> <p class="leading-7 mt-2">${job.notes || "Not provided"}</p> </div> <div class="mb-6"> <p class="font-medium flex items-center gap-x-2">${renderComponent($$result3, "User", User, { "size": 16 })} Assigned To</p> <p class="leading-7 mt-2"> ${job.assignedTo && job.assignedTo.name && job.assignedTo.email ? `${job.assignedTo.name || "Unknown"} ${job.assignedTo.surname || ""} (${job.assignedTo.email})` : "Not assigned"} </p> </div> <!-- 🔥 Novo bloco para mostrar quem criou o job --> ${job.createdBy && renderTemplate`<div class="mt-8 text-sm text-gray-500 border-t pt-4"> <p class="flex items-center gap-x-1">${renderComponent($$result3, "UserCircle", UserCircle, { "size": 16 })} By: ${job.createdBy.name} ${job.createdBy.surname} (${job.createdBy.email})</p> <p class="flex items-center gap-x-1 mt-1">${renderComponent($$result3, "CalendarDays", CalendarDays, { "size": 16 })} On: ${job.createdAt ? formatDate(job.createdAt) : "Not provided"}</p> </div>`} <div class="flex justify-end gap-x-4 mt-6"> <!-- Se o job não estiver atribuído, mostra botão "Assign" --> ${(userData.role === "level_01" || userData.role === "level_02") && (!job.assigned ? renderTemplate`<form method="post" action="/api/jobs/assign"> <input type="hidden" name="jobId"${addAttribute(job.id, "value")}> ${renderComponent($$result3, "Button", Button, { "type": "submit", "className": "bg-green-600 text-white cursor-pointer", "size": "sm" }, { "default": async ($$result4) => renderTemplate` ${renderComponent($$result4, "CheckCircle", CheckCircle, { "size": 16 })} Apply
